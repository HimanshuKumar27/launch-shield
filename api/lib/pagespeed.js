/**
 * api/lib/pagespeed.js — Google PageSpeed Insights API wrapper.
 * Ported from functions/pagespeed.js for Vercel Serverless Functions.
 * Uses PAGESPEED_API_KEY env var if set; otherwise returns deterministic mock data.
 */
...
function getMockPageSpeedData(url) {
  ...
  return {
    scores: {
      performance: perfScore, accessibility: a11yScore, seo: seoScore, bestPractices: bpScore,
    },
    performance: {
      fcp: `${fcpSec}s`, lcp: `${lcpSec}s`, speedIndex: `${siSec}s`,
      tbt: `${tbtMs}ms`, cls: String(clsVal),
      audits: [
        ...
        { id: 'render-blocking-resources', title: 'Eliminate render-blocking resources', score: perfScore >= 85 ? 0.9 : 0.4, displayValue: perfScore >= 85 ? '0.15s' : '0.98s', severity: perfScore >= 85 ? null : 'medium', description: 'Resources are blocking the first paint of your page.' },
    ...
    accessibility: {
      audits: [
        { id: 'image-alt',      title: 'Image elements have [alt] attributes',                              score: a11yScore >= 85 ? 1 : 0, severity: a11yScore >= 85 ? null : 'high', description: 'Some images are missing alternative text.' },
...
exports.audit = async function(url) {
  ...
  if (!apiKey) {
    console.warn('PAGESPEED_API_KEY not set — using deterministic mock data.');
    return getMockPageSpeedData(url);
  }

  const params = { url, strategy: 'mobile', category: CATEGORIES, key: apiKey };
  const response = await axios.get(PAGESPEED_API_BASE, { params, timeout: 90000 });
  ...
  return {
    ...
    performance: {
      ...
      audits:     extractAudits(audits, perfAuditIds).slice(0, 10),
    },
    accessibility: { audits: extractAudits(audits, a11yAuditIds).slice(0, 15) },
    seo:           { audits: extractAudits(audits, seoAuditIds).slice(0, 10)  },
    bestPractices: { audits: extractAudits(audits, bpAuditIds).slice(0, 10)   },
  };
};
